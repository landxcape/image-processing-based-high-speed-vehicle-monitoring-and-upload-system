...
# start video capture
cap = cv2.VideoCapture('Sallaghari4.mp4')
...
kernel = np.ones((7, 7), np.uint8)  # Taking a matrix as the kernel for dilation and erosion
...
while True:
    ...
    image_gray = myfn.grayScale(frame)
    ...
    # highspeed counter
    # cv2.putText(frame, str(highspeed_counter), (20, 20), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 255), 1)
    ...
    if live_centers:
        ...
        for i in range(len(old_centers)):
            ...
            # if live_centers[i][1] >= boundary_low >= old_centers[i][1] and old_centers[i][2] > max_speed_allowed:
